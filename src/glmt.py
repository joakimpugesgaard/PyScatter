...
class glmt:
    def __init__(self, maxJ, wavelength, nr, x, mu=1, mu1=1, dim = None):
        self.nr = nr
        self.maxJ = maxJ
        self.wl = wavelength
        ...
        # test wavelength is correct
        # Ensure no element in wavelength, nr, or R is negative
        if np.any(np.array(wavelength) <= 0):
            raise ValueError("All elements in wavelength must be greater than 0")
