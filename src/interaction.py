...
from src.displaced_beams import *
...
class interaction(multipoles):
    def __init__(self, beam, domain, nr, radius, mu = 1, mu1 = 1, interaction = "both"):
        ...
        self.polarization = self.beam_params["polarization"]
        ...
        #mie coefficients
        self.a = -self.mie.a_j() #minus following Bohren's expansion
        self.b = -self.mie.b_j()
        self.c = self.mie.c_j()
        self.d = self.mie.d_j()
    ...
    def compute_sum(self, includeBeam = False):
        assert self.a.ndim == 1, "a must be 1D"
        #initial multipole
        j0 = self.j0
        
        if self.beamtype == "focused":
            ...
            for j in range(j0+1, self.maxJ):
                ...
                mp_beam["magnetic"] += prefac[j] * mpb["magnetic"]
                mp_beam["electric"] += prefac[j] * mpb["electric"]
                
            sum_beam = mp_beam["magnetic"] + (1j) * self.p * mp_beam["electric"]
            sum = mp0["magnetic"] + (1j) * self.p * mp0["electric"]

        elif self.beamtype == "planewave":
            Js = np.arange(self.maxJ+1)
            coefs = 1j**Js * np.sqrt(2*Js+1)*np.sqrt(2*np.pi)
            prefac = (self.C.T * coefs).T

            if self.polarization == "left circular":
                mp0L = self.get_multipoles(j0, self.mz_star, self.spatial_fun, nr = self.nr)
                
                mp0L["magnetic"] *= prefac[j0] * (self.b[j0] * self.SCA + self.c[j0] * self.ABS)
                mp0L["electric"] *= prefac[j0] *(self.a[j0] * self.SCA + self.d[j0] * self.ABS)

                for j in range(j0+1, self.maxJ):
                    mp = self.get_multipoles(j, 1, self.spatial_fun, nr = self.nr)
                    
                    mp0L["magnetic"] += prefac[j] * (self.b[j] * self.SCA + self.c[j] * self.ABS)* mp["magnetic"] 
                    mp0L["electric"] += prefac[j] * (self.a[j] * self.SCA + self.d[j] * self.ABS)* mp["electric"]
                
                if includeBeam:
                    #sum_beam = self.beam.compute_sum()
                    sum_beam = np.zeros_like(mp0L["magnetic"], dtype=complex)
                    for i in range(len(self.planes)):
                        # LC
                        sum_beam[0][i] = np.exp(1j * self.k * self.Z[i])
                sum = mp0L["magnetic"]+ (1j) * 1 * mp0L["electric"]
            
            elif self.polarization == "right circular":
                mp0R = self.get_multipoles(j0, -1, self.spatial_fun)
            
                mp0R["magnetic"] *= prefac[j0] * (self.b[j0] * self.SCA + self.c[j0] * self.ABS)
                mp0R["electric"] *= prefac[j0] *(self.a[j0] * self.SCA + self.d[j0] * self.ABS)
        
                for j in range(j0+1, self.maxJ):
                    mp = self.get_multipoles(j, -1, self.spatial_fun)
                    
                    mp0R["magnetic"] += prefac[j] * (self.b[j] * self.SCA + self.c[j] * self.ABS)* mp["magnetic"] 
                    mp0R["electric"] += prefac[j] * (self.a[j] * self.SCA + self.d[j] * self.ABS)* mp["electric"]
                ...
                if includeBeam:
                    ...
                    for i in range(len(self.planes)):
                        # RC
                        sum_beam[2][i] = np.exp(1j * self.k * self.Z[i])
                sum = mp0R["magnetic"] - (1j)* 1 * mp0R["electric"]

            elif self.polarization == "x":
                mp0L = self.get_multipoles(j0, 1, self.spatial_fun)
                mp0R = self.get_multipoles(j0, -1, self.spatial_fun)
                mp0L["magnetic"] *= prefac[j0] * (self.b[j0] * self.SCA + self.c[j0] * self.ABS)
                mp0L["electric"] *= prefac[j0] *(self.a[j0] * self.SCA + self.d[j0] * self.ABS)
                mp0R["magnetic"] *= prefac[j0] * (self.b[j0] * self.SCA + self.c[j0] * self.ABS)
                mp0R["electric"] *= prefac[j0] *(self.a[j0] * self.SCA + self.d[j0] * self.ABS)
                for j in range(j0+1, self.maxJ):
                    mpL = self.get_multipoles(j, 1, self.spatial_fun)
                    mpR = self.get_multipoles(j, -1, self.spatial_fun)
                    mp0L["magnetic"] += prefac[j] * (self.b[j] * self.SCA + self.c[j] * self.ABS)* mpL["magnetic"] 
                    mp0L["electric"] += prefac[j] * (self.a[j] * self.SCA + self.d[j] * self.ABS)* mpL["electric"]
                    mp0R["magnetic"] += prefac[j] * (self.b[j] * self.SCA + self.c[j] * self.ABS)* mpR["magnetic"] 
                    mp0R["electric"] += prefac[j] * (self.a[j] * self.SCA + self.d[j] * self.ABS)* mpR["electric"]
                if includeBeam:
                    #sum_beam = self.beam.compute_sum()
                    sum_beam = np.zeros_like(mp0L["magnetic"], dtype=complex)
                    for i in range(len(self.planes)):
                        # LC
                        sum_beam[0][i] = np.exp(1j * self.wn * self.Z[i])/np.sqrt(2)
                        # RC
                        sum_beam[2][i] = np.exp(1j * self.wn * self.Z[i])/np.sqrt(2)
                sumL = mp0L["magnetic"] + (1j) * 1 * mp0L["electric"]
                sumR = mp0R["magnetic"] - (1j) * 1 * mp0R["electric"]
                ...
                if includeBeam:
                    ...
                    for i in range(len(self.planes)):
                        # LC
                        sum_beam[0][i] = np.exp(1j * self.wn * self.Z[i])/(1j*np.sqrt(2))
                        # RC
                        sum_beam[2][i] = -np.exp(1j * self.wn * self.Z[i])/(1j*np.sqrt(2))
                sumL = np.sqrt(2*np.pi)*(mp0L["magnetic"] + (1j) * 1 * mp0L["electric"])
                sumR = np.sqrt(2*np.pi)*(mp0R["magnetic"] - (1j) * 1 * mp0R["electric"])
                sum = (sumL - sumR) / (1j*np.sqrt(2))
            ...
            mp0 = self.get_multipoles(j0, self.mz_star, self.spatial_fun)
            mp_beam = self.get_multipoles(j0, self.mz_star, spatial_fun = "bessel")
            mp0["magnetic"] *= 0
            mp0["electric"] *= 0
            ...
            for j in range(j0, self.maxJ):
                ...
                for m in range(-j, j + 1):
                    ...
                    mpb = self.get_multipoles(j, self.mz_star, spatial_fun= "bessel")
                    mp0["magnetic"] += prefac[j, nn] *(b * self.SCA + c * self.ABS)* mp["magnetic"]
                    mp0["electric"] += prefac[j, nn] *(a * self.SCA + d * self.ABS)* mp["electric"]
                    
                    mp_beam["magnetic"] += prefac[j, nn] * mpb["magnetic"]
                    mp_beam["electric"] += prefac[j, nn] * mpb["electric"]
                    nn += 1
            sum_beam = mp_beam["magnetic"] + (1j) * self.p * mp_beam["electric"]
            sum = mp0["magnetic"] + (1j) * self.p * mp0["electric"]
            
        if includeBeam:
            # If includeBeam is True, return the sum and the beam multipoles
            return sum, sum_beam
        else:
            return sum
    
    def getCrossSection(self, type="scattering", dims = 1, includeParts = False, plot = True, verbose = True, **kwargs):
        ...
        Args:
            ...
            verbose (bool, optional): If True, prints constant parameters.
            **kwargs: Optional parameters to override x, radius, nr, or wavelength.
        ...
        # Store all kwargs for later use without modification
        ...
        if (xdim == 1):
            ...
            if radiusdim == 1:
                ...
                if verbose:
                    print("Size parameter and radius provided. Using only size parameter for 1D cross section.\n")
                wl = 2 * np.pi * radius / x
            elif wldim == 1:
                if  (len(x) != len(wl)):
        ...
        if not xdim == 1:
            if (radiusdim == 1) and (wldim == 1):
                ...
                if ((radiusdim == 1) and (nrdim == 1)):
                    if verbose:
                        print("Calculating new size parameter array from radius for 2D cross section with refractive index.\n")
                    x = 2 * np.pi * radius / wl
                    axis1 = radius
                    ...
                    if dims == 2:
                        if verbose:
                            print("Calculating new size parameter array from wavelength for 2D cross section with refractive index.\n")
                        axis2 = nr
                        axis2label = "Refractive Index (nr)"
                    elif dims == 1: 
                        if verbose:
                            print("Calculating new size parameter array from wavelength for 1D cross section with refractive index.\n")
                    x = 2 * np.pi * radius / wl
                    axis1 = wl
                    axis1label = "Wavelength (µm)"
                elif nrdim == 1:
                    if verbose:
                        print("Calculating new size parameter array from radius and wavelength for 1D cross section with refractive index.\n")
                    x = 2 * np.pi * radius / wl
                    axis1 = nr
                    ...
                    if verbose:
                        print("Calculating new size parameter array from radius for 1D cross section with refractive index.\n")
                    x = 2 * np.pi * radius / wl
                    axis1 = radius
                    axis1label = "Radius (µm)"
        ...
        if type == "scattering":
            ...
            #ensure C is 2D to make matrix multiplication work
            # Calculate internal cross section as difference between extinction and scattering
            
            Wext, included_e_ext, included_m_ext = self.getCrossSection(type="extinction", dims=dims, includeParts=True, plot=False, verbose=False, **original_kwargs)
            Wsca, included_e_sca, included_m_sca = self.getCrossSection(type="scattering", dims=dims, includeParts=True, plot=False, verbose=False, **original_kwargs)
            included_e = included_e_ext - included_e_sca
            included_m = included_m_ext - included_m_sca
        ...
        if dims == 1:
            for j in range(included_e.shape[0]):
                ...
                # scale 
                if not (np.isclose(max_e, 0) or np.isclose(max_m, 0)):
                    included_e[j] *= (np.max(W[idx_e]) / max_e)
                    included_m[j] *= (np.max(W[idx_m]) / max_m)

        if plot:
            if dims == 0:
                print("Cross section has dimension: 0")
    ...
    def plotInt(self, radius = None, nr = None, plot="components", globalnorm=False, includeBeam = False):
        ...
        if includeBeam:
            sum, sum_beam = self.compute_sum(includeBeam=True)
            # add the fields
            sum = sum*self.S + sum_beam*self.SCA # only field in r<R is the incident field
        else:
