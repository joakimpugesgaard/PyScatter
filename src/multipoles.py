...
class multipoles:
    def __init__(self, l_max, m_max, wavelength, domain, nr = 1, radius = None):
        self.l_max = l_max
        self.m_max = m_max
    ...
    def vsh(self, j, l, m, theta, phi):
        ...
        #Y = np.array([self.spharm(l, m_val, theta, phi) for m_val in M]) use scipy instead
    ...
    def get_multipoles(self, l, m, spatial_fun="hankel", nr = None):
        if nr is None:
            nr = self.nr    
        ...
        if spatial_fun == "hankel":
            ...
            spatial_func = lambda l, x: sp.spherical_jn(l, nr*x)
        elif spatial_fun == "both":
            spatial_func = lambda l, x: self.SCA*self.hankel(l, x) + self.ABS * sp.spherical_jn(l, nr*x)
    ...
    def plot_multipoles(self, l, m, type = "magnetic", interaction = "scattering", plot = "components", globalnorm = False):
        ...
        if plot == "components":
            ...
            for i, plane in enumerate(self.planes):
                ...
                im0 = axs[i, 0].imshow(np.abs(mp[type][0][i]).T, extent=(-0.5*self.size, 0.5*self.size, -0.5*self.size, 0.5*self.size), origin='lower', cmap='cmc.batlow', norm=norm)  
                ...
                im1 = axs[i, 1].imshow(np.abs(mp[type][1][i]).T, extent=(-0.5*self.size, 0.5*self.size, -0.5*self.size, 0.5*self.size), origin='lower', cmap='cmc.batlow', norm=norm)
                ...
                im2 = axs[i, 2].imshow(np.abs(mp[type][2][i]).T, extent=(-0.5*self.size, 0.5*self.size, -0.5*self.size, 0.5*self.size), origin='lower', cmap='cmc.batlow', norm=norm)
            ...
            for i, plane in enumerate(self.planes):
                im = axs[i].imshow(total_intensity[i].T, extent=(-0.5*self.size, 0.5*self.size, -0.5*self.size, 0.5*self.size), origin='lower', cmap='cmc.batlow', norm=norm)
